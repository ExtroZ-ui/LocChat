...
namespace serverChat
{
    class Program
    {
        ...
        private static void handlerCommands(string cmd) // обработка
        {
            cmd = cmd.ToLower();
        ...
        private static void startServer()  // Запуск сервера
        {
            ...
            socket.Listen(1000);
            Console.WriteLine("Сервер был запущен на IP: {0}.",ipEndPoint);
            while(true)
            {
                try
                {
